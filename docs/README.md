...
| **Home Assistant** | REST API + MQTT (192.168.1.54) | Power buttons switch HA entities via `switch.turn_on/off` (REST + long-lived token); MQTT mirrors station state |
...
- ⚙️ **Admin Settings screen** — edit MQTT (broker/port/user/pass), serial COM port & baud, rotator/tuner UDP IPs & ports, FlexRadio discovery/TCP ports, Home Assistant entity IDs, and general server settings; saved straight to `config.json`
...
│       ├── mqtt.js         #   Home Assistant MQTT status bridge
...
| `GET /api/settings` | admin | Read flattened, editable settings |
| `PUT /api/settings` | admin | Validate & save settings to config.json |
